...
API_KEY = "7278204931:AAG2_rg1yCJ-XLRRI5r3cBNaPidOkUY7XJ4"  # Telebot Token
URL = "https://api.monobank.ua/bank/currency"  # MonoBank API
...
class MenuBot:
    ...
    def menu_1(self, chat_id):
        ...
        button_other_currency = types.InlineKeyboardButton(
            "ІНШІ ВАЛЮТИ", callback_data="other currency"
        )
    ...
    def menu_2(self, chat_id):
        markup = types.InlineKeyboardMarkup(row_width=2)
        button_continue = types.InlineKeyboardButton(
...
class ConverterBot:
    ...
    def save_conversion_history(
        self, chat_id, from_currency, to_currency, amount, result
    ):
    ...
    def get_exchange_rate(self, currency_code):
        ...
        for item in data:
            if (
                ...
                and item.get("currencyCodeA") == currency_code
                and item.get("currencyCodeB") == 980
            ):
                if "rateSell" in item:
    ...
    def start_conversion(self, message):
        ...
        bot.send_message(
            message.chat.id, "Оберіть вихідну валюту:", reply_markup=markup
        )
    ...
    def amount_input(self, message):
        ...
        markup = self.menu_bot.menu_1(message.chat.id)
        bot.send_message(
            message.chat.id, "Оберіть цільову валюту:", reply_markup=markup
        )
    ...
    def result_conversation(self, call):
        ...
        rate_sell, rate_buy = self.get_exchange_rate(
            840 if target_currency == "USD" else 978
        )  # USD: 840, EUR: 978
        if rate_sell is None or rate_buy is None:
            bot.send_message(
                call.message.chat.id, "Вибачте, не можу знайти курс для даної валюти."
            )
            time.sleep(2)
            markup = self.menu_bot.menu_2(call.message.chat.id)
            self.send_message_with_markup(
                call.message.chat.id,
                "Бажаєте продовжити далі чи зупинити бота?",
                markup,
        ...
        self.save_conversion_history(
            call.message.chat.id, from_currency, target_currency, amount, final_amount
        )

        bot.send_message(
            call.message.chat.id,
            f"Результат конвертації: {final_amount:.2f} {target_currency}",
        )
        time.sleep(2)
    ...
    def continue_or_stop(self, call):
        ...
        if call.data == "continue":
            ...
            bot.send_message(
                call.message.chat.id, "Оберіть вихідну валюту:", reply_markup=markup
            )
        elif call.data == "end":
...
@bot.callback_query_handler(
    func=lambda call: call.data in ["continue", "end", "history"]
)
